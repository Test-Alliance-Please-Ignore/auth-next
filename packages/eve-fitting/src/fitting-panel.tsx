/**
 * Shared EVE circular fitting panel.
 *
 * Supports ship and pod display modes and relies on the caller for EVE image
 * URL generation so the package stays UI-framework agnostic.
 */
...
const SHIP_ARCS: Record<FittingShipSlotType, { r: number; a0: number; a1: number; n: number }> = {
...
const POD_ARCS: Record<'implant_top' | 'implant_bottom', { r: number; a0: number; a1: number; n: number }> = {
...
const SHIP_SLOT_POS: Record<FittingShipSlotType, SlotPos[]> = {
...
function LowSlotIcon() {
	return <line x1={11} y1={16} x2={21} y2={16} stroke={ICON_FILL} strokeWidth={2} strokeLinecap="round" />
}
...
function slotSeverityFill(severity?: FittingSlotHighlightMap[string]): string {
...
function SlotMarker({
	...
	pos: SlotPos
	icon: () => ReactElement
	severity?: FittingSlotHighlightMap[string]
}) {
	const Icon = icon
	return (
...
function groupShipItems(items: FittingDisplayItem[]) {
	const groups: Record<FittingShipSlotType, FittingDisplayItem[]> = {
		high: [],
		mid: [],
		low: [],
		rig: [],
	...
	for (const type of Object.keys(groups) as FittingShipSlotType[]) {
...
function countShipSlots(groups: Record<FittingShipSlotType, FittingDisplayItem[]>, slotCapacities?: FittingSlotCapacities) {
	return (Object.keys(groups) as FittingShipSlotType[]).reduce(
		(acc, type) => {
			const inferred = groups[type].reduce((max, item) => Math.max(max, item.slotIndex), -1) + 1
			const requested = slotCapacities?.[type]
			return {
				...acc,
				[type]: Math.min(SHIP_ARCS[type].n, Math.max(0, typeof requested === 'number' ? requested : inferred)),
			}
		},
		{} as Record<FittingShipSlotType, number>
	)
}
...
function ShipFittingPanel({
	...
	const groups = groupShipItems(items)
	const counts = countShipSlots(groups, slotCapacities)

	const slotIcons: Record<FittingShipSlotType, () => ReactElement> = {
		high: HighSlotIcon,
		mid: MidSlotIcon,
		low: LowSlotIcon,
		rig: RigSlotIcon,
	...
	return (
		<div className="flex justify-center">
			<div className="relative" style={{ width: P, height: P }}>
				<img
					src={getRenderUrl(shipTypeId, 512)}
				...
				>
					...
					{(Object.keys(SHIP_ARCS) as FittingShipSlotType[]).flatMap((type) =>
				...
				{(Object.keys(SHIP_ARCS) as FittingShipSlotType[]).flatMap((type) =>
					groups[type].slice(0, counts[type]).map((item) => {
						...
						return (
							<img
								...
								alt={item.typeName}
								title={item.quantity > 1 ? `${item.typeName} ×${item.quantity}` : item.typeName}
								className="absolute rounded"
...
function PodFittingPanel({
	...
	const implantItems = groupPodItems(items)
	...
	const implantByIndex = new Map(implantItems.map((item) => [item.slotIndex, item] as const))
