...
| Parse scan text (preview) | `POST /api/moon-scan/scans/parse` | `urn:moons:scan:submit` or site admin | No DB write |
| Submit scans | `POST /api/moon-scan/scans/submit` | `urn:moons:scan:submit` or site admin | High-sec systems filtered out |
| View own scans | `GET /api/moon-scan/scans/mine` | `urn:moons:scan:submit` or site admin | Uses user primary character ID |
| View validation queue | `GET /api/moon-scan/scans/queue` | `urn:moons:scan:validate` or site admin | Pending scans only |
| List scans (moderation list) | `GET /api/moon-scan/scans` | `urn:moons:scan:validate` or site admin | Supports status/moon/page filters |
| Verify scan | `POST /api/moon-scan/scans/:id/verify` | `urn:moons:scan:validate` or site admin | Updates verified composition |
| Reject scan | `POST /api/moon-scan/scans/:id/reject` | `urn:moons:scan:validate` or site admin | Marks scan rejected |
| Read scan by ID (verified) | `GET /api/moon-scan/scans/:id` | `urn:moons:view` OR `urn:moons:scan:submit` (owner only) OR `urn:moons:scan:validate` OR site admin | Owner access requires active submit URN |
| Read scan by ID (pending/rejected) | `GET /api/moon-scan/scans/:id` | `urn:moons:scan:submit` (owner only) OR `urn:moons:scan:validate` OR site admin | Owner-only access is not granted without submit URN |
...
| Show `Submit Scan` + `My Scans` nav | UI sidebar | `urn:moons:scan:submit` or site admin | Child links under Moon Scanning |
| Show `Validation Queue` nav | UI sidebar | `urn:moons:scan:validate` or site admin | Child link under Moon Scanning |
