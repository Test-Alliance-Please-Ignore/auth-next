...
describe('{{ pascalCase name }} Durable Object', () => {
	it('can increment counter', async () => {
		const id = env.{{ constantCase name }}.idFromName(`test-counter-${Date.now()}-${Math.random()}`)
		const stub = env.{{ constantCase name }}.get(id)
	...
	it('can get state', async () => {
		const id = env.{{ constantCase name }}.idFromName(`test-state-${Date.now()}-${Math.random()}`)
		const stub = env.{{ constantCase name }}.get(id)
	...
	it('can call example method', async () => {
		const id = env.{{ constantCase name }}.idFromName('test-example')
		const stub = env.{{ constantCase name }}.get(id)
