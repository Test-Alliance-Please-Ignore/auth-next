...
describe('createPasswordVerifier', () => {
	it('produces a contract-compliant pbkdf2-sha256 verifier', async () => {
		const verifier = await createPasswordVerifier('correct horse battery staple', 100_000)

		expect(verifier.algorithm).toBe('pbkdf2-sha256')
		expect(verifier.iterations).toBe(100_000)
	...
	it('defaults to the runtime-supported iteration ceiling', async () => {
	...
	it('uses a fresh salt per verifier', async () => {
		const a = await createPasswordVerifier('same-password', 100_000)
		const b = await createPasswordVerifier('same-password', 100_000)
	...
	it('derives deterministically for a fixed salt', async () => {
		...
		const verifier = await createPasswordVerifier(password, 100_000)
