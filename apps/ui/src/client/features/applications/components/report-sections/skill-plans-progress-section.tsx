...
function PlanProgressCard({
	...
	return (
		<Card variant="flat" className={cn('transition-colors border-l-4', statusColor)}>
					...
					{isLoading ? (
						...
						<div className="flex items-center justify-between">
							<div>
								...
								<p className="text-xs text-destructive">{t('hrpages.failedToLoadProgress')}</p>
							</div>
						...
						<div className="space-y-2">
									...
									{pctRequired >= 100 ? (
										pctRecommended >= 100 ? (
											<Badge variant="default" className="text-xs gap-1">
												...
												{t('hrpages.fullyTrained')}
											</Badge>
										) : (
											<Badge variant="secondary" className="text-xs gap-1">
												<CheckCircle2 className="h-3 w-3" />
												{t('hrpages.meetsRequired')}
											</Badge>
										)
									) : pctRequired >= 75 ? (
										<Badge variant="secondary" className="text-xs gap-1">
											<AlertCircle className="h-3 w-3" />
											{t('hrpages.almostReady')}
										</Badge>
									) : (
										<Badge variant="destructive" className="text-xs gap-1">
											<XCircle className="h-3 w-3" />
											{t('hrpages.trainingNeeded')}
										</Badge>
									)}
									{isExpanded ? (
							...
							<div className="grid grid-cols-2 gap-4">
								...
								<div className="space-y-0.5">
									<div className="flex justify-between text-xs text-muted-foreground">
										<span>{t('hrpages.recommended')}</span>
			...
			{isExpanded && progress && (
				<div className="border-t px-4 py-3 space-y-3">
					...
					<div className="rounded border">
						<div
							className={cn(
								'grid gap-x-3 px-3 py-1.5 border-b bg-muted/30 text-xs font-medium text-muted-foreground',
								SKILL_GRID_COLS
							)}
						>
							<span>{t('hrpages.skill')}</span>
							<span className="text-center">{t('hrpages.current2')}</span>
							<span className="text-center">{t('hrpages.required')}</span>
							<span className="text-center">{t('hrpages.rec')}</span>
							<span className="text-center">{t('hrpages.status')}</span>
						</div>
							...
							{(progress.skills || []).map((skill) => (
								<div
									...
									className={cn(
										...
										SKILL_GRID_COLS,
										skill.meetsRecommended ? 'text-muted-foreground' : 'text-foreground'
									)}
								>
									...
									<span className="text-center tabular-nums">{ROMAN[skill.requiredLevel]}</span>
									<span className="text-center tabular-nums">{ROMAN[skill.recommendedLevel]}</span>
...
export function SkillPlansProgressSection({ characterId }: SkillPlansProgressSectionProps) {
	...
	if (plans.length === 0) {
		return (
			<p className="text-sm text-muted-foreground py-4">
				{t('hrpages.noPublishedSkillPlansAvailable')}
			</p>
		)
	}
	...
	return (
		<div className="space-y-4">
			...
			<div className="grid gap-4 sm:grid-cols-3">
				<Card variant="flat">
					<CardContent className="py-3">
						<p className="text-xs text-muted-foreground">{t('hrpages.fullyTrained')}</p>
						<p className="text-lg font-bold text-green-500">{fullyTrainedCount}</p>
					</CardContent>
				</Card>
				<Card variant="flat">
					<CardContent className="py-3">
						<p className="text-xs text-muted-foreground">{t('hrpages.meetsRequired')}</p>
						<p className="text-lg font-bold text-yellow-500">{meetsRequiredCount}</p>
					</CardContent>
				</Card>
				<Card variant="flat">
					<CardContent className="py-3">
						<p className="text-xs text-muted-foreground">{t('hrpages.needsTraining')}</p>
			...
			{/* Search + Plan cards */}
			<Input
				placeholder={t('hrpages.searchSkillPlans')}
				value={search}
