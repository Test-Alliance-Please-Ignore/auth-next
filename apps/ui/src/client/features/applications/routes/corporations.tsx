...
const CORPORATION_TYPE_OPTIONS: Array<{
...
}> = [
	{
		value: 'member',
		get label() {
			return i18n.t('hrpages.memberCorps')
		},
	},
	{
		value: 'alt',
		get label() {
			return i18n.t('hrpages.altCorps')
		},
	},
	{
		value: 'special',
		get label() {
			return i18n.t('hrpages.specialPurposeCorps')
		},
	},
]
		...
		<div className="w-44 space-y-2">
			<div className="space-y-1">
				<div className="flex items-center justify-between text-xs text-muted-foreground">
					<span>{t('hrpages.authLinkedUsers')}</span>
					<span>
			...
			<div className="space-y-1">
				<div className="flex items-center justify-between text-xs text-muted-foreground">
					<span>{t('hrpages.charactersWithValidEsi')}</span>
					<span>
...
function CorporationCoverageBarsSkeleton() {
	...
	return (
		<div className="w-44 space-y-2" aria-label={t('hrpages.loadingEsiCoverage')}>
...
export default function CorporationsPage() {
	...
	}, [activeCorporationTypeFilter, canFilterCorporations, corporationSearch, corporations, t])
	const applicationCountsByCorporationId = useMemo(
		() => new Map(applicationCounts.map((counts) => [counts.corporationId, counts])),
		[applicationCounts, t]
	)

	usePageTitle(t('hrpages.corporations'))
				...
				<Card className="max-w-2xl mx-auto border-red-200 bg-red-50 dark:border-red-800 dark:bg-red-950">
					<CardHeader className="text-center">
						...
						<CardTitle className="text-2xl text-red-900 dark:text-red-100">
							{t('hrpages.failedToLoadCorporations')}
						</CardTitle>
			...
			<Container>
				<Card className="max-w-2xl mx-auto">
					<CardHeader className="text-center">
						...
						<CardTitle className="text-2xl">{t('hrpages.noCorporationAccess')}</CardTitle>
						<CardDescription className="mt-2">
							{t('hrpages.youDoNotCurrentlyHaveHrViewerReviewerAdminAccess')}
						</CardDescription>
					</CardHeader>
				</Card>
			</Container>
	...
	return (
		<Container>
			<PageHeader
				title={t('hrpages.corporations')}
				description={t('hrpages.selectACorporationToAccessMembersAndApplicationReviewTools')}
			/>
				...
				<Card className="mt-6">
					<CardContent className="flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:gap-6">
						<div className="relative min-w-0 sm:w-80 sm:shrink-0">
							...
							<Input
								aria-label={t('hrpages.filterCorporations')}
								placeholder={t('hrpages.filterCorporations2')}
			...
			{canFilterCorporations && visibleCorporations.length === 0 ? (
				<Card className="mt-6">
					<CardContent className="py-10 text-center text-muted-foreground">
						{t('hrpages.noCorporationsMatchTheSelectedFilters')}
					</CardContent>
				</Card>
			) : null}
							...
							<CardContent className="grid min-h-44 grid-cols-[minmax(0,1fr)_auto] grid-rows-[auto_1fr] gap-x-3 gap-y-4 p-6">
								<div className="min-w-0 self-start">
									...
									<CardDescription className="mt-1">
										{t('hrpages.corporationId')}
										{corporation.corporationId}
									</CardDescription>
								</div>
								<div className="justify-self-end self-start">
									{corporationAccessEntry?.userRole === 'CEO' ? (
										<Badge variant="warning">{t('hrpages.ceo')}</Badge>
									) : corporationAccessEntry?.userRole === 'Director' ? (
										<Badge variant="secondary">{t('hrpages.director')}</Badge>
									) : (
