...
const testBranchManager = new TestBranchManager({
	NEON_API_KEY: env.NEON_API_KEY!,
	NEON_PROJECT_ID: env.NEON_PROJECT_ID!,
})

beforeAll(async () => {
	await testBranchManager.beforeAll()
})
...
describe('Bills Integration Tests', () => {
	...
	beforeAll(async () => {
		...
		billsStub = getStub<Bills>(env.BILLS, 'default')
	})
	...
	describe('Bill Lifecycle', () => {
		it('should create a draft bill', async () => {
			...
			expect(bill.paymentToken).toBeTruthy()
			expect(bill.lateFee).toBe('0')
		})
