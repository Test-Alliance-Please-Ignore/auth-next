...
import type { ReportAlert, ResolvedCharacter } from '../../processors/alerts'
...
function sanitizeLegacyBlacklistSignals(
	conflicts: Record<string, unknown>
): Record<string, unknown> {
	const raw =
		conflicts &&
		typeof conflicts === 'object' &&
		conflicts.blacklistSignals &&
		typeof conflicts.blacklistSignals === 'object'
	...
	return {
		...
		ipAssociatedBlacklistedUsers: Array.isArray(raw.ipAssociatedBlacklistedUsers)
			...
			: [],
		matchedTargets: Array.isArray(raw.matchedTargets) ? raw.matchedTargets : [],
...
export async function generateAlerts(
	...
	try {
		...
		const [
			...
			safeRetrieve<
				| { transactions: ProcessedWalletTransaction[]; truncated?: boolean }
				| ProcessedWalletTransaction[]
		...
		if (publicInfo && fittedShips) {
			...
			if (customNames.size > 0) {
				...
				const { namesToResolve, candidateToCustomName } =
					extractCandidateCharacterNames(customNames)

				if (namesToResolve.length > 0) {
					const resolvedCharacters = await resolveNamesToCharacters(
						esiTypeResolverBinding,
						namesToResolve
					)
		...
		try {
					...
					if (namesToResolve.length > 0) {
						...
						shipNameCharIds = new Map(
							resolved.map((r) => {
								...
								return [
									String(r.characterId),
									{ customName: entry?.customName ?? r.name, characterName: r.name },
								] as const
							})
						)
					}
			...
			if (ipAssociations.matches.length > 0) {
				...
				const hashCount = new Set(ipAssociations.matches.flatMap((match) => match.matchingIpHashes))
					.size
